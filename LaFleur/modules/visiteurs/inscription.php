...
if (utilisateur_est_connecte()) {
	// On affiche la page d'erreur si l'utilisateur est déjà connecté   
...
else 
{
	...
	if ($form_inscription->is_valid($_POST)) { 
		// On vérifie si les 2 mots de passe sont égaux
	...
	if ($resultSsAr[4] > 10){// Verification de la vailiditer du date d'embauche
	
		$erreurs_inscription[] = "Le format de la date d'emboche et incorecte elle doit etre du type yyyy-mm-jj";
	
	}
	if( $resultSsAr[1]== false|| $resultSsAr[2]== false|| $resultSsAr[3] == false){ // Verification de la validite du code postal 
		
		
		$erreurs_inscription[] = "La date d'embauche et incorrecte, elle ne doit pas contenir des lettres, et etre de type AAAA-MM-JJ ";
		
	}
	
	if ($resultSsAr[1] <1977 ){
		$erreurs_inscription[] = "la date d''embauche ne peut pas etre inferieure a 1977 merci";
	}
	if ( $int_cp == false){
	
		$erreurs_inscription[] = "Code Postal invalide, il ne doit pas contenir des lettres";
		
	}
	if($int_cp[2] > 5){
	
		$erreurs_inscription[] = "Code Postal invalide";
	...
	//list($nom, $prenom,$dateEmb,$adresse,$codePostal,$ville, $mdp, $email, $avatar,$login) =
		...
		// Si on peut ajouter l'utilisateur (pas de doublons)
		$isarray = gettype($id_utilisateur);
		if ($isarray == 'string'){
		// Préparation du mail
		$message_mail = '<html><head></head><body>
		<p>Merci de vous etre inscrit sur le site de LaFleur</p>
		...
		echo "<p>$id_utilisateur , bienvenue sur le site de LaFleur</p><br/>";
